...
import app.api.service.KafkaProducerService;
...
class KafkaProducerServiceTest {
  ...
  void shouldSendMessageToKafkaSuccessfully() {
    DtoMessage testDtoMessage = new DtoMessage(UUID.randomUUID(), "SEND", "some message");
    ...
    records.iterator().forEachRemaining(
        record -> {
          ...
          try {
            message = objectMapper.readValue(record.value(), DtoMessage.class);
